...
const turnLight = (value) => {
  const headers = {
    ...
    'X-AIO-Key':'aio_JDqi619RdiLjvAWfd92202ZqrKqP'
  }
  const data2 = {
    "datum":
    {
  ...
  axios.post('https://io.adafruit.com/api/v2/Thientran/feeds/canhbaoled/data', data2, {
      headers: headers
    })
    .then((response) => {
      // console.log("OK")
...
module.exports.checkThreshold = async (context) => {
    if (context['record']) {
        ...
        if (newValue.value < min || newValue.value > max) {
            ...
            const options = {
              method: 'GET',
              url: `https://io.adafruit.com/api/v2/Thientran/feeds/canhbaoled`
            };
